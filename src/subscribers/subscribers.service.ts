...
async create(createSubscriberDto: CreateSubscriberDto , user : IUser) {
    ...
    if(isExist){
      throw new BadRequestException(`Subscriber với email = ${email} đã tồn tại`);
    }
  ...
  async findOne(id: string) {
    if(!mongoose.Types.ObjectId.isValid(id))
      return `Subscriber với id = ${id} không tồn tại`;
...
async getSkills(user : IUser){
  ...
  async update(updateSubscriberDto: UpdateSubscriberDto , user : IUser) {
    const updatedSubs = await this.subscriberModel.updateOne(
      { email : user.email },
      {
        ...updateSubscriberDto,
        updatedBy : {

...
export class JwtAuthGuard extends AuthGuard('jwt') {
  ...
  handleRequest(err, user, info , context: ExecutionContext) {
    ...
    if(targetEndpoint.startsWith("/api/v1/auth/"))  isExist = true; //bỏ qua kiểm tra quyền với các api auth

    if(!isExist && !isSkipPermission){
      throw new ForbiddenException("Bạn không có quyền truy cập quyền này");
    }
